...
def main(argv):
    ...
    train_seq = MNISTSequence(images=train_set[0], labels=train_set[1], batch_size=args.batch,
                              preprocessing=False)
    if 0:
        inputs = tfk.Input(shape=(IMAGE_SHAPE[0] * IMAGE_SHAPE[1],), name='img')
        kl_weight = 1.0 / (NUM_TRAIN_EXAMPLES / float(args.batch))
        output1 = tfp.layers.DenseLocalReparameterization(100,
                                                          activation="relu",
                                                          )(inputs)
        output = tfp.layers.DenseLocalReparameterization(NUM_CLASSES,
                                                         activation="softmax",
                                                         )(output1)
    ...
    model.fit(train_seq.images, train_seq.labels, epochs=args.num_epochs)
    heldout_seq = MNISTSequence(images=heldout_set[0], labels=heldout_set[1], batch_size=args.batch,
                                preprocessing=False)
    print(" ... Predicting")
    result_prob = model.predict(heldout_seq.images, batch_size=args.batch, verbose=1)
    ...
    if 0:
        logits = model(train_seq.images)
        neg_log_likelihood = tf.nn.softmax_cross_entropy_with_logits(
            labels=train_seq.labels, logits=logits)
        kl = sum(model.losses)
        loss = neg_log_likelihood + kl
        train_op = tf.keras.optimizers.Adam().minimize(loss)
    if 0:
    ...
    if 0:
        ...
        predicted_groups_probs = model_grouped.predict(x=heldout_seq_grouped.images, batch_size=args.batch, verbose=1)
